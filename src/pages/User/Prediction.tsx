...
// Data awal untuk riwayat prediksi
...
const PredictionPage: React.FC = () => {
  ...
  const [predictions, setPredictions] = useState(mockPredictionsData);
  ...
  const handleCreatePrediction = (e: React.FormEvent) => {
    ...
    console.log({ plantingDate, landArea /*, location */ });
    // Menambahkan prediksi baru ke state (contoh)
    const newPrediction = {
        id: (predictions.length + 1).toString(),
        name: `Prediksi Baru - ${new Date(plantingDate).toLocaleDateString('id-ID')}`,
        date: plantingDate
    };
